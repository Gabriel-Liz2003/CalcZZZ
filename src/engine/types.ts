...
export interface AnomalyDefinition { attribute:Attribute; threshold:number; baseMultiplier:number; duration:number; tickInterval?:number; disorderBaseMultiplier?:number; tickMultiplier?:number; maxTicks?:number; reactiveTicks?:boolean; meta:DataMeta; }
export interface AnomalyContribution { agentId:string; buildup:number; ap:number; atk:number; level:number; dmgBonus:number; pen:number; penRatio:number; resIgnore:number; }
...
export interface AnomalyState { buildup:Partial<Record<Attribute,number>>; contributions:Partial<Record<Attribute,AnomalyContribution[]>>; applications:Partial<Record<Attribute,number>>; active?:ActiveAnomaly; }
